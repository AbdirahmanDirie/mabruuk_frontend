...
const ThisMonthTransactions = () => {
  const {data} = useGetThisMonthkreportQuery();
...
let month = monthNames[date.getMonth()]; // getting the current month name
  ...
  return (
   ...
   <BsPrinter className='h-6 w-[6]'/> <p>Print PDF</p> 
      ...
      <div className='p-10 bg-white' ref={componentRef}>
        <p className='text-[23px] font-semibold text-[#489EE7] mb-6'>This Month Report <span className='text-black'> {month}</span></p>
        <DataTable
            columns={columns}
            data={data && data}
            dense
...
export default ThisMonthTransactions